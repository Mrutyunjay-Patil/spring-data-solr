...
public class SolrPageRequest implements Pageable {
	...
	public Pageable withPage(int pageNumber) {
		// Assuming SolrPageRequest has a constructor SolrPageRequest(int page, int size)
		// And an additional constructor SolrPageRequest(int page, int size, Sort sort) if sorting is supported
		if (this.isUnpaged()) {
			throw new UnsupportedOperationException("Cannot change page on an unpaged request.");
		}
		
		// Preserve the page size from the current instance
		int pageSize = this.getPageSize();
	
		// If your SolrPageRequest supports sorting, you might also need to preserve the sort information.
		// If so, use a constructor that also takes the sort parameter, something like:
		// return new SolrPageRequest(pageNumber, pageSize, this.getSort());
	
		// Return a new SolrPageRequest instance with the specified page number and the current page size
		return new SolrPageRequest(pageNumber, pageSize);
	}	
